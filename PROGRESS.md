...
- [~] **Phase 3 (partial, out of order)** — Unofficial YouTube video download, at the user's explicit request and informed consent after being told the trade-offs (not the official API -- verified no compliant automated download exists for any channel, including your own; this is the same category of technique Repurpose.io and similar tools almost certainly use). Off by default (`ENABLE_YOUTUBE_UNOFFICIAL_DOWNLOAD`). See `UNOFFICIAL_DOWNLOAD.md` for the full writeup. Verified against real videos from the connected channel (not just synthetic tests):
  - Hit YouTube's bot-detection wall immediately ("Sign in to confirm you're not a bot") on a plain public video -- fixed with yt-dlp's `player_client=android` spoof, a known community workaround, not invented here. Exactly the fragility that was flagged before building this; documented as the first thing to suspect if downloads start failing.
  - `download_one_video` produces a correctly-linked `MediaAsset` (sha256, ffprobe metadata, `match_confirmed=True` since there's no ambiguity -- downloaded directly by video ID) in one step; verified end-to-end against a real Short (360x640, 4s) from the connected channel, DB round-trip included.
  - Paced background job (`youtube_downloader`, one video per tick, default 15s) plus a per-video manual "Download" button in the Library UI. Per-video failure backoff (default 6h) so a permanently-broken video (private/deleted/age-restricted) doesn't get hammered every tick.
  - Full media/import fuzzy-matching UI (for files sourced outside this path -- Takeout exports, camera-roll originals) is still unbuilt; this only covers the "download it again via yt-dlp" path.
