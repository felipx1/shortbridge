...
- [~] **Phase 4** — TikTok OAuth (Login Kit). Code deployed and live, verified locally (`scripts/smoke_test_phase4.py`: authorize URL shape, mocked token-error parsing for both plausible TikTok response shapes, refresh-token rotation, unconfigured-provider refusal). **Blocked on real TikTok credentials** — see `TIKTOK_SETUP.md` for exact current console steps (Manage apps → Products → Login Kit + Content Posting API → Sandbox mode + target users). Key differences from Google baked into the implementation:
  - Scopes are comma-separated (Google's are space-separated) — an easy silent bug if copy-pasted from the Google provider without noticing.
  - No PKCE for a web server app (only required for desktop/mobile).
  - Refresh can rotate the refresh_token itself, unlike Google — `app.services.oauth` now persists a rotated token generically for any provider, was hardcoded Google-only before this phase.
  - `ProviderInvalidGrantError` (new shared base in `app/providers/base.py`) lets the reconnect-needed handling in `services/oauth.py` work for both providers through one code path instead of duplicating it.
  - TikTok's Sandbox mode (its "Testing" equivalent) has no Google-style 7-day token expiry, but does force all Content-Posting-API output to private until the app passes review — doesn't block OAuth itself, relevant starting Phase 5.
